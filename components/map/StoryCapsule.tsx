...
import type { DbCapsule, MoodTag, Track } from "./types";
...
export function StoryCapsule({
  ...
  /* ── Fetch capsules on place change ── */
  useEffect(() => {
    async function fetchCapsules() {
      try {
        setLoading(true);
        const res = await fetch(`/api/capsules?placeName=${encodeURIComponent(place.name)}`);
  ...
  const createShareLink = async () => {
    try {
      ...
      const res = await fetch("/api/share", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "city", placeName: place.name }),
      });
  ...
  return (
    <div
      ...
      style={{ background: "linear-gradient(160deg, #100d07 0%, #0e0b06 100%)" }}
    >
      {/* ── Header ── */}
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-3 [scrollbar-width:thin] [scrollbar-color:#3d2a0a_transparent]">
        ...
        {/* Form */}
        <CapsuleForm placeName={place.name} onSave={handleSave} saving={saving} />
        ...
        {!loading && capsules.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-sm font-semibold text-amber-200/50">No entries yet</p>
